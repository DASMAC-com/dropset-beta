...
\PROCEDURE{REGISTER-MARKET}{$input, insn, n\_accounts, insn\_len$}
  ...
  \ENDIF
  \STATE $pda\_seeds.base.addr = base\_mint.pubkey$
  \STATE $pda\_seeds.base.len =$ \texttt{Address.size}
  \COMMENT{Get the input buffer, incremented by the base mint padded data length.}
  \STATE $input\_shifted = input + base\_mint.padded\_data\_len$
  \COMMENT{Check quote mint account, create signer seed.}
  \IF{$quote\_mint.duplicate \neq$ \texttt{input\_buffer.NON\_DUP\_MARKER}}
    \RETURN \texttt{ErrorCode::QuoteMintIsDuplicate}
  \ENDIF
  \STATE $pda\_seeds.quote.addr = quote\_mint.pubkey$
  \STATE $pda\_seeds.quote.len =$ \texttt{Address.size}
  \COMMENT{Advance pointer to next account.}
  % This should actually be a different name
  \STATE $input\_shifted = input\_shifted +  quote\_mint.padded\_data\_len$
  \STATE $input\_shifted = input\_shifted +$ \texttt{EmptyAccount.size}
  \COMMENT{Derive market PDA, check it matches the market account pubkey.}
  \STATE \CALL{Store}{$input$}
  \STATE $syscall.seeds = pda\_seeds$
  \STATE $syscall.program\_id = program\_id$
