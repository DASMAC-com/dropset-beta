...
/// Expand `#[instruction("target")]` on a struct into:
/// - The original struct
/// - `impl StructName { pub const INSN_LEN: u64 = ...; }`
...
pub fn expand(target: &str, input: &syn::ItemStruct) -> proc_macro2::TokenStream {
    ...
    let asm_name = format!("{}_INSN_LEN", screaming);
    ...
    let meta_def = codegen::immediate_meta(
        ...
        &asm_name,
        &doc,
        quote! { super::#struct_name::INSN_LEN as i32 },
    );
